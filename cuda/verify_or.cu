...
__global__ void or_kernel(const float* A, const float* B, unsigned char* out, size_t n) {
    size_t idx = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (idx < n) {
        float a = A[idx];
        float b = B[idx];
        out[idx] = (unsigned char)((a!=0.0f)||(b!=0.0f) ? 1 : 0);
    }
}
...
int main(int argc, char** argv) {
    ...
    or_kernel<<<blocks, threads>>>(d_a, d_b, d_out, n);
